...
func (g *DefaultGnome) Query(userQuery string) (string, error) {
...
func NewGnome(logger *log.Logger, model glue.Model) DefaultGnome {
	result := DefaultGnome{
		systemPrompt: groqSystemPrompt,
		sqlTemplate:  groqSqlTemplate,
		model:        model,
		logger:       logger, // used in glue.Request
	}
